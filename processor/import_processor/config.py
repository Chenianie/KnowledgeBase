...
class ImportConfig:
    ...
    # ==================== 文档处理配置 ====================
    max_content_length: int = 1500 # 切片最大长度
    img_content_length: int = 200  # 图片上下文最大长度
    min_content_length: int = 500  # 合并短内容的最小长度
    max_title_length: int = 100  # 标题/父标题/文件标题最大字符长度（与Milvus集合VARCHAR字段max_length保持一致，切分节点按此截断兜底）
    ...
    bge_m3_name :str = field(
        default_factory=lambda: os.getenv("BGE_M3", "")
    )
    bge_m3_device :str = field(
        default_factory=lambda: os.getenv("BGE_DEVICE", "")
    )
    bge_fp16 :bool = field(
        default_factory=lambda: os.getenv("BGE_FP16", "True").strip().lower() == "true"
    )
    @classmethod
