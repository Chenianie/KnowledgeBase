    ...
    def _step_2_generate_embeddings(self, chunks: List[Dict[str, str]]) -> List[Dict[str, str]]:
        ...
        for i in range(0, len(chunks), batch_size):
            ...
            for j,doc in enumerate(batch_chunks):
                doc['dense_vector'] = docs_embeddings['dense'][j]
                doc['sparse_vector'] = docs_embeddings['sparse'][j]
                output_data.append(doc)
            self.logger.info(f"成功获取第 {i + 1}-{min(i + len(batch_chunks), len(chunks))} 项的嵌入。")
...
if __name__ == "__main__":
    ...
    json_path = r"D:\output\H3C LA2608室内无线网关 用户手册-6W100-整本手册\chunks_item.json"
