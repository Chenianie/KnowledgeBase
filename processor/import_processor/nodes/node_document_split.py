...
class NodeDocumentSplit(BaseNode):
    ...
    def _step_1_get_inputs(self, state:ImportGraphState) -> Tuple[str , str]:
        ...
        if not md_content:
            raise StateFieldError(field_name="md_content", message="MD文件内容不能为空", expected_type=str)
    ...
    def _step_2_split_by_titles(self, content: str, file_title: str) -> Tuple[List[Dict[str, str]], int, int]:
        ...
        for line in lines:
            ...
            #     is_code_block = not in_code_block
            ...
            # 4.2 识别标题
            is_valid_title = (not in_code_block ) and re.match(title_param,line)
            if is_valid_title:
                ...
                title_count += 1
                self.logger.debug(f"识别标题：{current_title}")
            else:
                # 普通行
                current_lines.append(line)
        _flush_section()
...
if __name__ == '__main__':
    setup_logging()
    md_path = r"D:\output\H3C LA2608室内无线网关 用户手册-6W100-整本手册\H3C LA2608室内无线网关 用户手册-6W100-整本手册_new.md"
