...
3. 如果无法识别商品名称,请返回空字符串。
    ...
    def _step_5_generate_vectors(self, item_name: str) -> Tuple[List, Dict]:
        ...
        vectors = generate_embeddings([item_name]) #需要的是list,所以要[]
    ...
    def _step_6_save_to_milvus(self, state: ImportGraphState, file_title: str, item_name: str, dense_vector,
        ...
        核心逻辑：
            ...
            3. 幂等性处理：删除同 file_title 旧数据，避免重复导入同一文件时堆积重复记录
        ...
        try:
            ...
            # 3. 幂等性处理：删除同文件标题旧数据（避免重复导入同一文件时堆积重复记录）
            # 清理粒度与 node_import_milvus 保持一致：按 file_title 清理，同一商品的多份文档（如说明书+安全手册）互不覆盖
            # 转义文件标题（防止特殊字符导致filter解析失败）
            safe_file_title = escape_milvus_string(file_title)
            # 构建过滤表达式：file_title等于目标值
...
if __name__ == '__main__':
    setup_logging()
    md_path = r"D:\output\H3C LA2608室内无线网关 用户手册-6W100-整本手册\chunks.json"
    ...
    res = node_item_name_recognition(init_state)

    logging.getLogger().info(json.dumps(res,indent=4,ensure_ascii=False))
