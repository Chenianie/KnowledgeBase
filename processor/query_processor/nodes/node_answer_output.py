...
from processor.utils.mongo_history_utils import save_chat_message
from processor.utils.sse_utils import push_to_session, SSEEvent
from processor.utils.task_utils import set_task_result, add_done_task
...
class NodeAnswerOutput(NodeBase):
    ...
    def process(self, state: QueryGraphState) -> QueryGraphState:
        ...
        5 做最后一次push操作（主要是为了触发前端图片渲染)
             {
                "answer": "HAK 180 热金机的操作面板位于...（大模型生成的纯文本）...",
                "status": "completed",
                "image_urls": [
    ...
    def _format_chat_history(self, chat_history: List[Dict], char_budget: int) -> Tuple[str, int]:
        ...
        for message in chat_history:
            ...
            if used_chars + len(formatted_line) + 1 > char_budget:
                break

            formatted_lines.append(formatted_line)
            used_chars += len(formatted_line) + 1  # +1 预留 \n
...
if __name__ == "__main__":
    ...
    mock_reranked_docs = [
        {
            "chunk_id": "local_101",
            "source": "local",
            "title": "HAK 180 热金机操作手册_v2.pdf",
            "score": 0.95,
            "content": """
            HAK 180 热金机的操作面板位于机器正前方。
            开启电源后，您需要先设置温度，默认建议设置在 110℃ 左右。
            具体的操作面板布局请参考下图：
            ![操作面板布局图](http://192.168.10.169:9000/knowledge-base/upload-images/AolynkCB304nCable网桥用户手册-5W100-整本手册/3e9494b93e191e1d0f60f036d56898ce66cd2fe843d14d2832b1ad6169e2f5b3.jpg)

            如果是进行局部烫金，请调节侧面的旋钮。
            ![侧面旋钮细节](http://192.168.10.169:9000/knowledge-base/upload-images/AolynkCB304nCable网桥用户手册-5W100-整本手册/20ca17710916fb3f05c58702df31faf6e798a2beb1db672b505bb9a5f8d1bee0.jpg)
            """
        },
        {
            "chunk_id": None,
            ...
            "score": 0.88,
            "url": "http://192.168.10.169:9000/knowledge-base/upload-images/AolynkCB304nCable网桥用户手册-5W100-整本手册/a56768e1d9c1fa782ddaf0b692ac9d1e17418acb96a0fd81eef24037704b2bb7.jpg", # 这是一个直接指向图片的URL（虽然少见，但用于测试提取）
    ...
    mock_history = [
        ...
        {"role": "user", "text": "HAK 180 热金机"}
    ]

    # 模拟输入状态
    mock_state = {
        "session_id": "test_answer_session_001",
        "original_query": "HAK 180 热金机怎么操作？",
        "rewritten_query": "HAK 180 热金机的具体操作步骤和面板设置方法",
        "item_names": ["HAK 180 热金机"],
