...
from processor.utils.json_format_utils import format_json
from processor.utils.task_utils import add_done_task
...
class NodeRrf(NodeBase):
    ...
    def process(self, state: QueryGraphState) -> QueryGraphState:
        ...
        :return: 更新后的状态对象
        """
        embedding_chunks_list = [doc.get('entity') for doc in state.get("embedding_chunks", []) if isinstance(doc, dict)]
        # embedding_chunks_list = []
        # docs = state.get("embedding_chunks",[])
        # if docs:
        #     for doc in docs:
        #         if isinstance(doc, dict):
        #             embedding_chunks_list.append(doc.get('entity'))

        hyde_embedding_chunks_list = [doc.get('entity') for doc in state.get("hyde_embedding_chunks", []) if isinstance(doc, dict)]
    ...
    def _rrf_merge(self, rrf_inputs, k: int = 60, max_results: int = None) -> List[Tuple[Dict[str, Any], float]]:
        ...
        # 按得分降序排序 chunk_scores 中有id和分数, unsorted_results 中有文档和分数
        unsorted_results = [(chunk_data[cid],score) for cid,score in chunk_scores.items()]
        # 排序时看每个元素的第 2 个值（也就是分数）sorted(要排列的列表, key=排序的依据, reverse=是否降序)
