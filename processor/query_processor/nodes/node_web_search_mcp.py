...
from processor.query_processor.config.bailian_mcp_config import mcp_config
...
class NodeWebSearchMcp(NodeBase):
    ...
    def process(self, state: QueryGraphState) -> QueryGraphState:
        ...
        # 如果没有查询内容，直接返回
        if query:
            result = asyncio.run(self._mcp_call(query)) # 调用 MCP 搜索接口
            if result:
                print(result)
...
if __name__ == "__main__":
    ...
    init_state = {
        "rewritten_query": "关于HAK180烫金机，如何调节转印温度？"
    }
