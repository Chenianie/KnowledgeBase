...
from processor.query_processor.config import lm_config
...
class NodeItemNameConfirm(NodeBase):
    ...
    def _step_4_extract_info(self, query, history) -> Dict:
        ...
        try:
            ...
            # 9. 给item_names 去除空格
            result['item_names'] = [name.replace(' ','').replace('\n','').replace('\r','').replace('\t','') for name in result['item_names']]
...
if __name__ == "__main__":
    ...
    # 将返回的图状态进行json序列化
    json_state = format_json(result, ensure_ascii=False, indent=4)
    # 输出
