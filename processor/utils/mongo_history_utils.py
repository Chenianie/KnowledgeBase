...
class HistoryMongoTool:
    ...
    def __init__(self):
        ...
        try:
            ...
            logging.info(f"Successfully connected to MongoDB: {self.mongo_db_name}")
...
if __name__ == "__main__":
    ...
    # # 1. 写入用户消息
    # save_chat_message(sid, "user", "你好，有无限控制器吗？")
    # # 2. 写入助手回复
    # save_chat_message(sid, "assistant", "你好！请问你想询问哪个型号？")
    # # 3. 写入带关联商品的用户消息
    # save_chat_message(sid, "user", "LA2608 的")
    # save_chat_message(sid, "assistant", "有的")
...
#     save_chat_message(sid, "assistant", "有的")
    ...
    # 1. 写入用户消息
    save_chat_message(sid, "user", "你好，有wifi吗？")
    # 2. 写入助手回复
    save_chat_message(sid, "assistant", "你好！请问你想询问哪个型号？")
    # 3. 写入带关联商品的用户消息
    save_chat_message(sid, "user", "H3CLA2608室内无线网关")
    save_chat_message(sid, "assistant", "有的")
